...
describe('buildTimeline', () => {
  it('단판은 spinning → revealed 로 1.8초를 넘지 않는다', () => {
    const timeline = buildTimeline(1);
    expect(timeline.map((s) => s.phase)).toEqual(['spinning', 'revealed']);
    expect(duration(timeline)).toBe(SINGLE_MS.spinning);
    expect(duration(timeline)).toBeLessThanOrEqual(1_800);
  });
  ...
  it('3판이어도 총 연출 시간이 3.3초를 넘지 않는다', () => {
    expect(duration(buildTimeline(3))).toBeLessThanOrEqual(3_300);
  });
...
describe('frameAt', () => {
  it('단판의 경계 직전·직후로 단계가 넘어간다', () => {
    ...
    expect(frameAt(0, timeline).phase).toBe('spinning');
    expect(frameAt(total - 1, timeline).phase).toBe('spinning');
    expect(frameAt(total, timeline).phase).toBe('revealed');
  });

  it('삼세번은 판 인덱스가 함께 넘어간다', () => {
    const timeline = buildTimeline(2);
    expect(frameAt(0, timeline)).toMatchObject({ phase: 'spinning', round: 0 });
    expect(frameAt(SERIES_MS.spinning, timeline)).toMatchObject({
      phase: 'roundResult',
      round: 0,
    });
    expect(frameAt(SERIES_MS.spinning + SERIES_MS.roundResult, timeline)).toMatchObject({
      phase: 'spinning',
      round: 1,
    });
  });
...
describe('revealedCount', () => {
  it('연출 중에는 끝난 판까지만, revealed에서는 전부 센다', () => {
    const timeline = buildTimeline(3);
    expect(timeline.map((s) => revealedCount(s, 3))).toEqual([0, 1, 1, 2, 2, 3]);
  });
});
