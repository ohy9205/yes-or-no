...
- **공유 범위: 결과 카드 이미지 저장 + 텍스트 공유** — Canvas로 카드를 그려 `saveBase64Data`로 기기 저장, `share`로 텍스트 공유
...
사용할 SDK API (전부 `@apps-in-toss/web-framework`에서 named export).
**아래는 설치된 `@apps-in-toss/web-bridge` 타입 정의로 실제 시그니처를 확인한 값입니다** — 초안에 적었던 `Share.sendMessage` / `Share.createLink` / `File.saveBase64` 같은 네임스페이스 형태는 SDK에 존재하지 않습니다:

- `share({ message }): Promise<void>` — 텍스트 공유 (이미지 미지원)
- `getTossShareLink(path, ogImageUrl?): Promise<string>` — `intoss://` 딥링크를 포함한 토스 공유 링크 생성. **인자는 객체가 아니라 위치 인자**
- `saveBase64Data({ data, fileName, mimeType }): Promise<void>` — 기기에 파일 저장 (`data:` 프리픽스 제거한 순수 base64)
- `generateHapticFeedback({ type }): Promise<void>` — 햅틱 진동. `type`은 `tickWeak | tap | tickMedium | softMedium | basicWeak | basicMedium | success | error | wiggle | confetti`
...
│  ├─ features/
│  │  ├─ haptic.ts                   # generateHapticFeedback 안전 래퍼
│  │  ├─ decision/
│  │  │  ├─ decide.ts                # 순수 랜덤 로직
│  │  │  ├─ decide.test.ts
...
│  │  │  ├─ renderResultCard.test.ts
│  │  │  ├─ saveResultImage.ts       # saveBase64Data 래퍼
│  │  │  ├─ shareResult.ts           # getTossShareLink + share
│  │  │  └─ shareResult.test.ts
│  │  └─ storage/
...
│  │  ├─ RevealStage.tsx             # ... → ??? 연출
│  │  ├─ ResultCard.tsx
│  │  └─ ResultActions.tsx           # 다시 하기 / 공유 / 이미지 저장
│  └─ styles/
│     ├─ theme.ts
│     └─ stage.ts                    # 연출·결과 공유 레이아웃
```
...
- 질문 줄바꿈은 어절 단위로 끊되, **한 어절이 한 줄보다 길면 글자 단위로 쪼갭니다**. 띄어쓰기 없는 한국어 질문이 카드 밖으로 넘치는 것을 막기 위함이고, 최대 3줄을 넘으면 말줄임합니다.
- **이미지 저장** — `data:image/png;base64,` 프리픽스를 잘라내고 `saveBase64Data({ data, fileName: 'yesno-<timestamp>.png', mimeType: 'image/png' })`
- **텍스트 공유** — `getTossShareLink('intoss://<appName>')`로 링크를 받아 `share({ message })`에 조합:
  ```
  오늘 치킨 먹을까?
  ...
  <toss link>
  ```
- 두 API 모두 구버전 토스 앱/권한 거부 시 실패할 수 있으므로 `try/catch`로 감싸고 실패 시 토스트만 띄우고 앱은 계속 동작하게 합니다. 딥링크 생성만 실패한 경우에는 링크를 빼고 텍스트 공유를 이어갑니다.
...
3. **실기기(토스 테스트앱/샌드박스)** — SDK 호출은 브라우저에서 검증 불가하므로 반드시 실기기에서:
   - `saveBase64Data` 후 기기 갤러리에 카드 이미지가 실제로 생기는지
   - `share`가 토스 공유 시트를 띄우는지
   - `generateHapticFeedback`이 실제로 진동하는지
...
- ~~**햅틱 피드백**: 앱인토스 SDK의 햅틱 API 존재 여부~~ → **해결(Phase 4)**. `generateHapticFeedback`이 `@apps-in-toss/web-framework`에서 정상 export됩니다. 적용 완료.
- **`getTossShareLink`의 `ogImageUrl`**: 결과 이미지를 OG 이미지로 넣으려면 공개 URL이 필요한데 서버가 없습니다. MVP에서는 인자를 생략했고, 필요하면 콘솔에 업로드한 고정 대표 이미지 URL을 두 번째 인자로 넘기면 됩니다.
- **`appName` 확정값** — Phase 0 완료 시 `granite.config.ts`와 `src/features/share/shareResult.ts`의 `APP_NAME` 상수에 함께 반영해야 합니다(현재 둘 다 `yes-or-no`).
...
- yes/no가 룰렛 효과처럼 돌아가다가 멈춤

> **한쪽 확률 높이기 / 삼세번** — 보류를 풀고 구현 완료했습니다. `plan-options.md` 참고.